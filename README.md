...
#### 13. Adjust the brightness of the image.
```
...
#### 14. Create brighter and darker images.
```
...
#### 15. Display the images (Original Image, Darker Image, Brighter Image).
```
...
#### 17. Display the images (Original, Lower Contrast, Higher Contrast).
```
plt.figure(figsize=(12,4))
plt.subplot(1,3,1); plt.imshow(boy_rgb); plt.title("Original"); plt.axis("off")
...
#### 18. Split the image (boy.jpg) into the B,G,R components & Display the channels.
```
...
#### 19. Merged the R, G, B , displays along with the original image
```
...
#### 20. Split the image into the H, S, V components & Display the channels.
```
...
```
#### 21. Merged the H, S, V, displays along with original image.
```
